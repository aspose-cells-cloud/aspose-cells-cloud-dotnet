...
namespace Aspose.Cells.Cloud.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeCellsCloudContext.atcc = new AsposeCellsCloudContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeCellsCloudContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeCellsCloudContext.Current.Cache.Add(
				   SessionID,
					resources,
